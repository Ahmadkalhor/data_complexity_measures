...
class Data_Domain_Scoring_Unsupervised:
...
def module_data_domain_scoring_unsupervised(data, data_test, black_hole=False, normalize_do=False):
    n_max_clusters=100
    device = data.device
    ...
    if normalize_do:
        instant_intial = Data_Domain_Scoring_Unsupervised(data, black_hole)
        mean_data, scale_matrix=instant_intial.get_normalization_matrix()
        shift_matrix=mean_data.repeat([n_data,1])
        data=(data-shift_matrix)@scale_matrix
        shift_matrix=mean_data.repeat([n_data_test,1])
        data_test=(data_test-shift_matrix)@scale_matrix

    #=============================================
    instant = Data_Domain_Scoring_Unsupervised(data, black_hole)
    ...
    if n_cluster_h>1:
        self_score=1-values[:,0]/values[:,1]
    else:
        self_score=1-values[:,0]/values.max()
    self_score=self_score.mean()
    #==========================================================================
    distance=torch.cdist(data_test,center_clusters,p=2)
    values, indices= distance.sort(1)
    if n_cluster_h>1:
        cross_score=1-values[:,0]/values[:,1]
    else:
        cross_score=1-values[:,0]/values.max()
    cross_score=cross_score.mean()
   #==========================================================================
    print('Self Score Index is: ', self_score.detach().cpu().numpy())
    print('Cross Score Index is: ', cross_score.detach().cpu().numpy())
    data=data.cpu()
